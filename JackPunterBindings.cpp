#if !defined(FCODER_JACK_PUNTER_BINDINGS)
#define FCODER_JACK_PUNTER_BINDINGS

function void
JackPunterGlobalBinding(Mapping *mapping, i64 global_id)
{
    MappingScope();
    ...
    Bind(jp_macro_toggle_recording,     KeyCode_U, KeyCode_Control);
    ...
    Bind(jp_switch_buffer_other,        KeyCode_I, KeyCode_Control, KeyCode_Shift);
...
function void
JackPunterFileBinding(Mapping *mapping, i64 global_id, i64 file_id)
{
    MappingScope();
    SelectMapping(mapping);
...
function void
JackPunterCodeBinding(Mapping *mapping, i64 file_id, i64 code_id)
{
    MappingScope();
    SelectMapping(mapping);
    ...
    Bind(move_left_alpha_numeric_boundary,           KeyCode_Left, KeyCode_Control, KeyCode_Shift);
    Bind(move_right_alpha_numeric_boundary,          KeyCode_Right, KeyCode_Control, KeyCode_Shift);
...
function void
JackPunterBindings(Mapping *mapping)
{
    // NOTE(jack): Set up default mappings and do some overrides.
    //setup_default_mapping(mapping, mapid_global, mapid_file, mapid_code);
    JackPunterGlobalBinding(mapping, mapid_global);
    JackPunterFileBinding(mapping, mapid_global, mapid_file);
    JackPunterCodeBinding(mapping, mapid_file, mapid_code);
}

#endif // FCODER_JACK_PUNTER_BINDINGS
